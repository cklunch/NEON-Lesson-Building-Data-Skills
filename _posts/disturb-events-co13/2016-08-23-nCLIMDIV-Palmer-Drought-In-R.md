...
with the 
<a href="{{ site.basurl }}/teaching-module/disturb-event-co13/overview" target="_blank"> Ecological Disturbance Teaching Modules. 
...
like to pre-download the data set it can be downloaded from the <a href="https://ndownloader.figshare.com/files/6780978"> NEON Data Skills account
on FigShare</a>.  
...
Our data have a header (remember the sample file!) - the first row represents the 
...
Oops, that doesn't work!  R returned an origin error. The Date class is built on
having  day, month, and year data. `R` needs a day of the month in order to properly 
convert this to a date class. The origin error is saying it doesn't know where 
